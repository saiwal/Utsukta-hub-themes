	<div id="thread-wrapper-{{$item.id}}" class="thread-wrapper{{if $item.toplevel}} {{$item.toplevel}} generic-content-wrapper card h-entry mb-4 {{else}} u-comment h-cite card-footer text-body-secondary{{/if}}" data-b64mids='{{$item.mids}}'>
		...
		<div class="wall-item-outside-wrapper{{if $item.is_comment}} comment{{/if}}{{if $item.previewing}} preview{{/if}}" id="wall-item-outside-wrapper-{{$item.id}}" >
            ...
            {{if $item.title_tosource}}{{if $item.plink}}<a href="{{$item.plink.href}}" class="{{if $item.is_new}}text-white{{/if}} text-decoration-none" title="{{$item.title}} ({{$item.plink.title}})" rel="nofollow">{{/if}}{{/if}}{{$item.title}}{{if $item.title_tosource}}{{if $item.plink}}</a>{{/if}}{{/if}}
				</div>
				{{if ! $item.is_new}}
				<hr class="m-0">
				{{/if}}
				{{/if}}
				<div class="p-2 wall-item-head{{if $item.is_new && !$item.event && !$item.is_comment}} wall-item-head-new{{/if}} mb-2" >
