...
class ChannelActivities
{
    ...
    private function files(string $category): void
    {
        ...
        $i = [];
        foreach ($r as $rr) {
            // Link the containing folder by path — the SPA's file view resolves
            // /cloud/<nick>/<folder path> into its folder stack (core links the
            // file itself with a #<id> anchor the SPA has no use for).
