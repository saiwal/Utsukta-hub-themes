...
`loadedDraftId` is the thread that ties a loaded draft to cleanup: `submit()` deletes it automatically — no caller-side bookkeeping needed. It deletes it *before* posting, not after: a draft is an `item` row with the same body, and core's "Suppress duplicates" matches any item of the uid with an identical body from the last 2 minutes, unpublished included, so a surviving draft cancels its own post ("operation cancelled"). If the post then fails, `loadedDraftId` is already null and the next autosave creates a fresh draft. Canceling does *not* delete the draft (the pending key was already consumed, but the server-side draft item stays put for next time).
