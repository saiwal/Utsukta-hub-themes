...
<div id="hide-comments-outer-{{$item.parent}}" class="hide-comments-outer fakelink small"
	onclick="showHideComments({{$item.id}});">
	<i id="hide-comments-icon-{{$item.id}}" class="bi bi-chevron-down align-middle hide-comments-icon"></i> <span
		id="hide-comments-label-{{$item.id}}" class="hide-comments-label align-middle"
		data-expanded="{{$item.collapse_comments}}"
		data-collapsed="{{$item.expand_comments}}">{{$item.expand_comments}}</span>{{if !$item.threaded}}&nbsp;<span
		id="hide-comments-total-{{$item.id}}" class="hide-comments-label align-middle">{{$item.num_comments}}</span>{{/if}}
</div>
	...
	<div id="thread-wrapper-{{$item.id}}"
				...
				<div class="p-2 wall-item-head{{if !$item.title && !$item.event && !$item.photo}} rounded-top{{/if}} clearfix">
					<div class="lh-sm text-end float-end">
						<div class="wall-item-ago text-body-secondary" id="wall-item-ago-{{$item.id}}">
							...
							{{/if}}
							<small class="autotime" title="{{$item.isotime}}"><time class="dt-published"
									datetime="{{$item.isotime}}">{{$item.localtime}}</time>{{if
								$item.expiretime}}&nbsp;{{$item.expiretime}}{{/if}}</small>
						</div>
						{{if !$item.threaded && $item.thr_parent_uuid}}
						<a href="javascript:doscroll('{{$item.thr_parent_uuid}}',{{$item.parent}});" class="ms-3"
							title="{{$item.top_hint}}"><i class="bi bi-chevron-double-up"></i></a>
						{{/if}}
						{{if $item.pinned}}
						<div class="wall-item-pinned" title="{{$item.pinned}}" id="wall-item-pinned-{{$item.id}}"><i
								class="bi bi-pin-fill"></i></div>
						{{/if}}
					</div>
					<div class="float-start wall-item-info pe-2" id="wall-item-info-{{$item.id}}">
						<div class="wall-item-photo-wrapper{{if $item.owner_url}} wwfrom{{/if}} h-card p-author"
							id="wall-item-photo-wrapper-{{$item.id}}">
							{{if $item.contact_id}}
							<div class="spinner-wrapper contact-edit-rotator contact-edit-rotator-{{$item.contact_id}}">
								<div class="spinner s"></div>
							</div>
							{{/if}}
							<img src="{{$item.thumb}}" class="fakelink wall-item-photo{{$item.sparkle}} u-photo p-name"
								id="wall-item-photo-{{$item.id}}" alt="{{$item.name}}" loading="lazy" data-bs-toggle="dropdown" />
							...
							<div class="dropdown-menu">
								...
								<a class="dropdown-item{{if $mitem.class}} {{$mitem.class}}{{/if}}" {{if
									$mitem.href}}href="{{$mitem.href}}" {{/if}} {{if $mitem.action}}onclick="{{$mitem.action}}" {{/if}}
									{{if $mitem.title}}title="{{$mitem.title}}" {{/if}}{{if $mitem.data}}
									{{$mitem.data}}{{/if}}>{{$mitem.title}}</a>
								{{/foreach}}
							</div>
							{{/if}}
						</div>
					...
					<div class="wall-item-author">
						...
						<div class="float-start dropdown wall-item-lock">
							<i class="bi {{if $item.locktype == 2}}bi-envelope{{else if $item.locktype == 1}}bi-lock{{else}}bi-unlock{{/if}} lockview{{if $item.privacy_warning}} text-danger{{/if}}"
								data-bs-toggle="dropdown" title="{{$item.lock}}" onclick="lockview('item',{{$item.id}});"></i>&nbsp;
				...
				<div class="p-2 wall-item-tools d-flex flex-wrap justify-content-between">
					<div class="wall-item-tools-left hstack gap-1" id="wall-item-tools-left-{{$item.id}}">
						...
						<a href="{{$item.viewthread}}" target="_thread" title="{{$response.count}} {{$response.button.label}}"
							class="btn btn-sm btn-link{{if !$item.observer_activity.$verb}} link-secondary{{/if}} wall-item-{{$response.button.class}}"
						...
						{{else}}
						<button type="button" title="{{$response.count}} {{$response.button.label}}"
							class="btn btn-sm btn-link{{if !$item.observer_activity.$verb}} link-secondary{{/if}} wall-item-reaction wall-item-{{$response.button.class}}"
							...
							data-item-reaction-count="{{$response.count}}">
						...
						<div class="">
							<button type="button" class="btn btn-sm btn-link link-secondary" data-bs-toggle="dropdown"
								id="wall-item-react-{{$item.id}}">
							...
							<div class="dropdown-menu dropdown-menu-start container text-center w-25">
								<div class="row g-0">
									...
									<div class="col-3 p-2">
										<a class="" href="#" onclick="jotReact({{$item.id}},'{{$react}}'); return false;"><img
												class="menu-img-1" src="/images/emoji/emojitwo/{{$react}}.png" alt="{{$react}}" /></a>
									</div>
									{{/foreach}}
								</div>
							</div>
					...
					<div class="wall-item-tools-right hstack gap-1" id="wall-item-tools-right-{{$item.id}}">
						...
						{{if $item.attachments}}
						<div class="">
							<button type="button" class="btn btn-sm btn-link link-secondary wall-item-attach"
								data-bs-toggle="dropdown" id="attachment-menu-{{$item.id}}"><i
									class="bi bi-paperclip generic-icons"></i></button>
							<div class="dropdown-menu dropdown-menu-end">{{$item.attachments}}</div>
						</div>
						{{/if}}
						{{if $item.reply_to}}
						<button type="button" title="{{$item.reply_to.0}}" class="btn btn-sm btn-link link-secondary"
						...
						<div class="">
							...
							<div class="dropdown-menu dropdown-menu-end" role="menu" aria-labelledby="wall-item-menu-{{$item.id}}">
								...
								{{/if}}
								{{if $item.tagger}}
								<a class="dropdown-item" href="#" onclick="itemTag({{$item.id}}); return false;"><i
										id="tagger-{{$item.id}}" class="generic-icons-nav bi bi-tag"
										title="{{$item.tagger.tagit}}"></i>{{$item.tagger.tagit}}</a>
								{{/if}}
								{{if $item.filer}}
								...
								{{/if}}
								{{if $item.star}}
								<a class="dropdown-item" href="#" onclick="dostar({{$item.id}}); return false;"><i
										id="starred-{{$item.id}}"
								...
								{{/if}}
								{{if $item.expand}}
								<a class="dropdown-item dropdown-item-expand" href="#" data-item-id="{{$item.id}}"
									data-item-uuid="{{$item.mid}}"><i id="expand-{{$item.id}}"
										class="generic-icons-nav bi bi-arrows-angle-expand"
										title="{{$item.expand}}"></i>{{$item.expand}}</a>
								{{/if}}
		...
		<div id="load-more-progress-wrapper-{{$item.id}}" class="progress{{if $item.blog_mode}} d-none{{/if}}"
			...
			<div id="load-more-progress-{{$item.id}}" class="progress-bar bg-info"
				...
				data-comments-total="{{$item.comments_total}}"></div>
		</div>
		<div id="load-more-{{$item.id}}"
			...
			<span id="load-more-dots-{{$item.id}}" class="load-more-dots rounded"><span class="dot-1">-</span> <span
