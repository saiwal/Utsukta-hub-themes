<label id="{{$field.0}}_container" class="d-flex justify-space-between">
  <span for="id_{{$field.0}}">{{$field.1}}{{if $field.6}}<sup class="required zuiqmid">{{$field.6}}</sup>{{/if}}</span>
    <input type="checkbox" class="" name="{{$field.0}}" id="id_{{$field.0}}" value="1" {{if $field.2}}checked="checked" {{/if}} {{if $field.5}}{{$field.5}}{{/if}} />
</label><br>
