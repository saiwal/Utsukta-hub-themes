<ul class="flex-column" style="list-style:none;">
	...
	<li class="p-2"{{if $p.id}} id="{{$p.id}}"{{/if}}>
		<a class="{{if $p.sel}} link-secondary {{$p.sel}}{{/if}}" href="{{$p.url}}"{{if $p.title}} title="{{$p.title}}"{{/if}}{{if $p.sub}} onclick="{{if $p.sel}}closeOpen('{{$p.id}}_sub');{{else}}openClose('{{$p.id}}_sub');{{/if}} return false;"{{/if}}>
