...
<div id="hide-comments-outer-{{$item.parent}}" class="d-flex justify-content-center hide-comments-outer fakelink small"
  onclick="showHideComments({{$item.id}});"><span class="badge text-bg-info">
    <i id="hide-comments-icon-{{$item.id}}" class="bi bi-chevron-down align-middle hide-comments-icon"></i> <span
      ...
      id="hide-comments-total-{{$item.id}}"
      class="hide-comments-label align-middle">{{$item.num_comments}}</span>{{/if}}</span>
</div>
  ...
  <div id="thread-wrapper-{{$item.id}}"
    ...
    <div class="wall-item-outside-wrapper{{if $item.is_comment}} comment{{/if}}{{if $item.previewing}} preview{{/if}}"
      ...
      <div class="rounded wall-item-content-wrapper{{if $item.is_comment}} comment{{/if}}"
        ...
        {{if $item.photo}}
        <div class="wall-photo-item entry__content-media" id="wall-photo-item-{{$item.id}}">
          {{$item.photo}}
          {{/if}}
          {{if $item.event}}
          <div class="wall-event-item entry__header border-bottom-0" id="wall-event-item-{{$item.id}}">
            ...
            <div class="entry__header wall-item-title " id="wall-item-title-{{$item.id}}">
              {{if $item.title_tosource}}{{if $item.plink}}<a href="{{$item.plink.href}}"
                ...
                $item.title_tosource}}{{if $item.plink}}</a>{{/if}}{{/if}}
            </div>
            {{/if}}
            <div class="entry__meta d-flex align-items-center">
              <ul>
                  ...
                  <div class="row g-0">
                    ...
                    <div class="col-3 p-2">
                      <a class="" href="#" onclick="jotReact({{$item.id}},'{{$react}}'); return false;"><img
                          class="img-size-32" src="/images/emoji/emojitwo/{{$react}}.png" alt="{{$react}}" /></a>
                    </div>
                    {{/foreach}}
                  </div>
                </div>
              ...
              <div class="">
                ...
                <div class="dropdown-menu dropdown-menu-end" role="menu" aria-labelledby="wall-item-menu-{{$item.id}}">
                  ...
                  <a class="dropdown-item" href="#"
                    onclick="jotEmbed({{$item.id}},{{$item.item_type}}); return false"><i
