<div id="side-bar-photos-albums">
	...
	<ul class="link-list">
		...
		{{if $al.shorttext}}
		<li ><a class="" href="{{$baseurl}}/photos/{{$nick}}/album/{{$al.bin2hex}}"><span class="badge bg--primary float-end">{{$al.total}}</span>{{$al.shorttext}}</a></li>
		{{/if}}
		{{/foreach}}
		{{/if}}
	</ul>
