<div class="generic-content-wrapper">
  <div class="section-title-wrapper app-content-header">
    {{if $editor}}
    <div class="float-end">
      <button id="webpage-create-btn" class="btn btn-sm btn-success" onclick="openClose('layout-editor');"><i
          class="bi bi-pencil-square-o"></i>&nbsp;{{$create}}</button>
      <a href="{{$help.url}}" target="_blank" class="btn btn-sm btn-warning" title="{{$help.title}}"><i
          class="bi fa-info"></i>&nbsp;{{$help.text}}</a>
    </div>
    {{/if}}
    <h3>{{$title}}</h3>
    <div class="clear"></div>
  ...
  <div id="pagelist-content-wrapper" class="section-content-wrapper-np">
    <div class="mb-3">
      <table id="layout-list-table" class="table">
        <tr>
          <th width="1%">{{$name}}</th>
        ...
        <tr id="layout-list-item-{{$item.url}}">
          ...
          <td class="webpage-list-tool">
            {{if $edit}}
            <a href="#" title="{{$delete}}"
              onclick="dropItem('item/drop/{{$item.url}}', '#layout-list-item-{{$item.url}}'); return false;"><i
                class="bi bi-trash drop-icons"></i></a>
            {{/if}}
          </td>
