    ...
    <button class="btn btn-outline-secondary d-none" type="button" id="messages-file-input-clear">
      <i class="bi bi-x-lg"></i>
		...
		<a href="{{$e.href}}" class="list-group-item list-group-item-action message" data-b64mid="{{$e.b64mid}}">
			...
			{{if $e.unseen_count}}
      <span class="badge bg-{{$e.unseen_class}} border border-{{$e.unseen_class}} border-1 text-body-{{$e.unseen_class}} rounded-pill position-absolute bottom-0 end-0 m-2 unseen_count" title="{{$strings.unseen_count}}">{{$e.unseen_count}}</span>
			{{/if}}
		</a>
		{{/foreach}}
...
$('#messages-file').on('change', function(e) {
  ...
  $('#messages-container .message').remove();
  $('#messages-file-container').addClass('sticky-top');
...
$(document).on('click', '#messages-file-input-clear', function() {
  $('#messages-file').val('');
  $("#messages-file").prop('selectedIndex', 0); // reset to placeholder
