...
<div id="messages-widget" class="overflow-auto clearfix card border-top-0 mb-3" style="height: 70vh;">
	...
	<div id="messages-container" class="list-group list-group-flush" data-offset="10">
		...
		<a href="{{$e.href}}" class="list-group-item list-group-item-action message" data-b64mid="{{$e.b64mid}}">
			...
			{{if $e.unseen_count}}
			<span class="badge bg-secondary border  text-body-{{$e.unseen_class}} rounded-pill position-absolute bottom-0 end-0 m-2 unseen_count" title="{{$strings.unseen_count}}">{{$e.unseen_count}}</span>
			{{/if}}
		</a>
		{{/foreach}}
			...
			if (obj.entries.length) {
				obj.entries.forEach(function(e) {
					html = tpl.format(
						...
						e.href,
						e.icon,
						e.unseen_count ? '<span class="badge bg-secondary border ' + ' text-body-' + e.unseen_class + ' rounded-pill position-absolute bottom-0 end-0 m-2 unseen_count" title="{{$strings.unseen_count}}">' + e.unseen_count + '</span>' : '',
						e.author_img
					);
