...
function update_inroom(inroom) {
  ...
  html.setAttribute('class', 'contacts-list');
	...
	$.each( inroom, function(index, item) {
		...
		$(newNode).html('<img src="'+ item.img +'" alt="'+ item.name +'" class="contacts-list-img img-size-32"><div class="contacts-list-info"><span class="contacts-list-name text-secondary">'+ item.name +'<small class="contacts-list-date text-success float-end">' + item.status + '</small></span></div>');
