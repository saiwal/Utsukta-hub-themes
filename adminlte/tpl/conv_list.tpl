						...
						<div class="btn-group">
							{{if $item.like}}
                                <button type="button" title="{{$item.like.0}}" class="btn btn-secondary btn-sm" onclick="dolike({{$item.id}},'like'); return false;">
                                    <i class="bi bi-hand-thumbs-up{{if $item.my_responses.like}} ivoted{{/if}}" ></i>
                                </button>
                                {{/if}}
                                {{if $item.dislike}}
                                <button type="button" title="{{$item.dislike.0}}" class="btn btn-secondary btn-sm" onclick="dolike({{$item.id}},'dislike'); return false;">
                                    <i class="bi bi-hand-thumbs-down{{if $item.my_responses.dislike}} ivoted{{/if}}" ></i>
                                </button>
                                {{/if}}
                                {{if $item.isevent}}
                                <div class="btn-group">
                        ...
                        <div class="wall-item-tools-left btn-group"  id="wall-item-tools-left-{{$item.id}}">
                            ...
                            <div class="wall-item-list-comments btn-group">
                                <a class="btn btn-secondary btn-sm border-0 wall-item-comments" href="{{$item.viewthread}}" title="{{$item.comment_count_txt.label}}{{if $item.list_unseen_txt}}, {{$item.list_unseen_txt.label}}{{/if}}">
                                    <i class="bi bi-comment-o"></i> {{$item.comment_count_txt.count}}{{if $item.unseen_comments}}<span class="unseen-wall-indicator-{{$item.id}}">, <i class="bi bi-eye-slash"></i> {{$item.list_unseen_txt.count}}</span>{{/if}}
                                </a>
                            </div>
                            ...
                            {{if $response.count}}
                            <div class="btn-group">
                            <button type="button" class="btn btn-secondary btn-sm border-0 wall-item-{{$response.button.class}}"{{if $response.modal}} data-bs-toggle="modal" data-bs-target="#{{$verb}}Modal-{{$item.id}}"{{else    }} data-bs-toggle="dropdown"{{/if}} id="wall-item-{{$verb}}-{{$item.id}}"><i class="bi bi-{{$response.button.icon}}"></i> {{$response.count}}</button>
                                {{if $response.modal}}
