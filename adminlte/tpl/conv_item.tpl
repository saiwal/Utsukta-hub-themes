	...
	<div id="thread-wrapper-{{$item.id}}" class="thread-wrapper{{if $item.toplevel}} {{$item.toplevel}} card {{if $item.is_new}} card-outline card-success{{/if}} generic-content-wrapper h-entry mb-4{{else}} u-comment h-cite card-footer border-top-0 text-body-secondary ps-1 pe-0{{/if}} clearfix{{if $item.is_contained}} is-contained{{/if}}{{if $item.is_new && !$item.event && !$item.photo && !$item.title && !$item.is_comment}} is-new{{/if}}" data-b64mids='{{$item.mids}}'>
		...
		<div class="wall-item-outside-wrapper{{if $item.is_comment}} comment{{/if}}{{if $item.previewing}} preview{{/if}}" id="wall-item-outside-wrapper-{{$item.id}}" >
			<div class="rounded wall-item-content-wrapper{{if $item.is_comment}} comment{{/if}}" id="wall-item-content-wrapper-{{$item.id}}">
				...
				<div class="card-header wall-item-title border-bottom-0" id="wall-item-title-{{$item.id}}">
            ...
            <a href="{{$item.viewthread}}" target="_thread" title="{{$response.count}} {{$response.button.label}}" class="btn btn-sm btn-link{{if !$item.observer_activity.$verb}} link-secondary{{/if}} wall-item-{{$response.button.class}}" id="wall-item-{{$verb}}-{{$item.id}}">
							<i class="bi bi-chat{{if $item.observer_activity.$verb}}-fill{{/if}} generic-icons"></i>{{if $response.count}}<span style="display: inline-block; margin-top: -.25rem;" class="align-top">{{$response.count}}</span>{{/if}}
						</a>
						{{else}}
		...
		<div id="load-more-{{$item.id}}" class="load-more text-center text-secondary {{if $item.blog_mode}} d-none{{/if}}" title="{{$item.load_more_title}}" onclick="request(0, '{{$item.rawmid}}', 'load', {{$item.parent}}, ''); return false;">
			<span id="load-more-dots-{{$item.id}}" class="loadmore-dots badge rounded-pill text-bg-info">{{$item.load_more_title}}</span>
		</div>
		{{/if}}
