...
function widget_topnavbar($args)
{
  ...
  if (local_channel()) {
    ...
    if (empty($_SESSION['delegate']) && feature_enabled(
      local_channel(),
      'nav_channel_select'
    )) {
      $chans = q("select channel_name, channel_id
  ...
  if ($observer) {
    $userinfo['icon'] = $observer['xchan_photo_s'] . '?rev='
      . strtotime($observer['xchan_photo_date']);
    $userinfo['icon_m'] =
    ...
    $userinfo['icon_l'] =
      ...
      strtotime($observer['xchan_photo_date']);
    $userinfo['icon_mime_type'] =
      $observer['xchan_photo_mimetype'];
    $userinfo['addr'] =
      $observer['xchan_addr'];
  ...
  if ($channel) {
    ...
    $userinfo['theme'] =
      $channel['channel_theme'];
    $userinfo['timezone'] =
      $channel['channel_timezone'];
    $userinfo['startpage'] =
  ...
  if (!$is_owner) {
    $nav['rusermenu'] = [
      ...
      t('Take me home'),
      'logout',
      ((local_channel()) ? t('Logout') : t('Log me out of this
        site'))
    ];
  }
  ...
  if (false /* !Config::Get('system', 'hide_help') */) {
    $help_url =
      z_root() . '/help?f=&cmd=' . App::$cmd;
    $context_help        = '';
    $enable_context_help = ((intval(Config::Get(
      'system',
      'enable_context_help'
    )) === 1 || Config::Get(
      'system',
   ...
   */
  if (is_site_admin()) {
    $nav['admin'] = ['admin/', t('Admin'), "", t('Site
  ...
  if (
    App::$profile_uid && isset(App::$nav_sel['raw_name']) &&
    App::$nav_sel['raw_name']
  ) {
    $active_app = q(
      "SELECT app_url FROM app
    ...
    if ($active_app) {
      if (strpos($active_app[0]['app_url'], ',')) {
        $urls =
          explode(',', $active_app[0]['app_url']);
        $url  = trim($urls[0]);
        if ($is_owner) $settings_url = trim($urls[1]);
      } else {
        $url =
  ...
  if ($is_owner) {
    ...
    if (
      get_pconfig(local_channel(), 'system', 'force_import_system_apps')
      !== STD_VERSION
    ) {
      Apps::import_system_apps();
      set_pconfig(
  ...
  if ($pinned_list) {
    foreach ($pinned_list as $app) {
      ...
      if ($is_owner) {
        ...
        !$is_owner && strpos($app['requires'], 'local_channel') ===
        false
      ) {
        $navbar_apps[] = Apps::app_render($app, 'navbar');
      }
  ...
  if ($syslist) {
    foreach ($syslist as $app) {
      ...
      } elseif (!isset($app['requires']) || (isset($app['requires']) &&
  ...
  $c   = theme_include('navbar_' . purify_filename($template) .
    '.css');
  $tpl = get_markup_template('navbar_' .
  ...
  App::$page['topnav'] .= replace_macros($tpl, [
    ...
    get_pconfig(
      ...
      'system',
      'channel_menu',
      Config::Get('system', 'channel_menu')
    ),
    ...
    '$thumb'              => ((!$is_owner && isset(App::$profile['thumb'])) ?
      App::$profile['thumb'] : ''),
    '$form_security_token' =>
