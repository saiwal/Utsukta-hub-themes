...
function widget_mynavbar($args)
{
    ...
    App::$page["htmlhead"] .= '<script>$(document).read(function() {$("#search-input").modal_search_autocomplete(\'' . z_root() . "/acl" . '\', "#searchResultsModal");});</script>';
    ...
    if (local_channel()) {
        ...
        $nav["usermenu"][] = [
            ...
            t('View
          Profile'),
            isset(App::$nav_sel["raw_name"]) &&
            App::$nav_sel["raw_name"] == "Profile"
                ? "active"
                : "",
            t('Your
          profile page'),
    ...
    if (!$is_owner) {
        $nav["rusermenu"] = [
            ...
            local_channel()
                ? t("Logout")
                : t('Log me out of this
        site'),
        ];
    }

    if (
        (Config::Get("system", "register_policy") == REGISTER_OPEN ||
    ...
    if (false /* !Config::Get('system', 'hide_help') */) {
        ...
        $context_help = "";
        $enable_context_help =
            intval(Config::Get("system", "enable_context_help")) === 1 ||
            Config::Get("system", "enable_context_help") === false
                ? true
                : false;
    ...
    if (isset(App::$page["color_mode"])) {
        $theme_switch_icon =
            App::$page["color_mode"] === "dark" ? "sun" : "moon";
    }
    ...
    if ($is_owner) {
        if (
            get_pconfig(local_channel(), "system", "import_system_apps") !==
    ...
    if ($pinned_list) {
        foreach ($pinned_list as $app) {
            if (
                isset(App::$nav_sel["name"]) &&
                App::$nav_sel["name"] == $app["name"]
            ) {
                $app["active"] = true;
            }
    ...
    App::$page["topnav"] .= replace_macros($tpl, [
        ...
        '$name' =>
            !$is_owner && isset(App::$profile["fullname"])
