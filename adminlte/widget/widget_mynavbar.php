...
function widget_mynavbar($args) {
    ...
    if (local_channel()) {
        ...
        if (!get_account_id()) {
            if (App::$module === 'channel') {
                ...
                App::$page['content'] .= replace_macros(get_markup_template('nav_login.tpl'), ['$nav' => $nav, 'userinfo' => $userinfo]);
            }
        } else {
    ...
    if (!$is_owner) {
        $nav['rusermenu'] = [$homelink, t('Take me home'), 'logout', ((local_channel()) ? t('Logout') : t('Log me out of this site'))];
    }
    ...
    if ($pinned_list) {
        foreach ($pinned_list as $app) {
            if (isset(App::$nav_sel['name']) && App::$nav_sel['name'] == $app['name']) $app['active'] = true;
            if ($is_owner) {
    ...
    // Render the template once with all data, including notifications
    App::$page['topnav'] = replace_macros($tpl, [
        ...
        '$loading' => t('Loading'),
        '$sys_only' => empty($args['sys_only']) ? 0 : 1
    ]);

    if (x($_SESSION, 'reload_avatar') && $observer) {
        // The avatar has been changed on the server but the browser doesn't know that, force the browser to reload the image
        $tpl = get_markup_template('force_image_reload.tpl');
        App::$page['topnav'] .= replace_macros($tpl, ['$imgUrl' => $observer['xchan_photo_m']]);
