<ul class="flex-column list-unstyled">
	...
	<li class="p-2"{{if $p.id}} id="{{$p.id}}"{{/if}}>
		<a class="nav-link{{if $p.sel}} {{$p.sel}} fw-semibold{{/if}}" href="{{$p.url}}"{{if $p.title}} title="{{$p.title}}"{{/if}}{{if $p.sub}} onclick="{{if $p.sel}}closeOpen('{{$p.id}}_sub');{{else}}openClose('{{$p.id}}_sub');{{/if}} return false;"{{/if}}>
		...
		{{if $p.sub}}
		<ul class="flex-column ml-4 list-unstyled" id="{{$p.id}}_sub"{{if !$p.sel}} style="display: none;"{{/if}}>
			{{foreach $p.sub as $ps}}
			<li class="nav-item"{{if $ps.id}} id="{{$ps.id}}"{{/if}}>
				<a class="nav-link{{if $ps.sel}} {{$ps.sel}} fw-semibold{{/if}}" href="{{$ps.url}}"{{if $ps.title}} title="{{$ps.title}}"{{/if}}>
