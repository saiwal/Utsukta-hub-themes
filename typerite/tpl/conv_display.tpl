  ...
  <div class="wall-item-outside-wrapper{{if $item.is_comment}} comment{{/if}}{{if $item.previewing}} preview{{/if}}"
    ...
    <div class="rounded wall-item-content-wrapper{{if $item.is_comment}} comment{{/if}}"
      ...
      <div class="entry__header mb-3">
        ...
        <h1 class="wall-item-title " id="wall-item-title-{{$item.id}}">
      ...
      <div class="p-2 pt-1 pb-1 wall-item-tools d-flex justify-content-between">
        <div class="wall-item-tools-left hstack gap-1" id="wall-item-tools-left-{{$item.id}}">
          ...
          <a href="{{$item.viewthread}}" target="_thread" title="{{$response.count}} {{$response.button.label}}"
            class="p-4{{if !$item.observer_activity.$verb}} link-secondary{{/if}} wall-item-{{$response.button.class}}"
          ...
          <a type="button" title="{{$response.count}} {{$response.button.label}}"
            class="p-4{{if !$item.observer_activity.$verb}} link-secondary{{/if}} wall-item-reaction wall-item-{{$response.button.class}}"
        ...
        <div class="wall-item-tools-right hstack gap-1" id="wall-item-tools-right-{{$item.id}}">
          ...
          <div class="" id="star-button-{{$item.id}}">
            <a type="button" class="p-4 link-secondary wall-item-star" onclick="dostar({{$item.id}});"><i
                class="bi bi-star generic-icons"></i></a>
          </div>
          {{/if}}
          {{if $item.attachments}}
          <div class="comment__meta">
            <a type="button" class="p-4 link-secondary wall-item-attach" data-bs-toggle="dropdown"
            ...
            <div class="dropdown-menu dropdown-menu-end" role="menu" aria-labelledby="wall-item-menu-{{$item.id}}">
              ...
              <a class="dropdown-item dropdown-item-expand" href="#" data-item-id="{{$item.id}}"
                data-item-uuid="{{$item.mid}}"><i id="expand-{{$item.id}}"
                  class="generic-icons-nav bi bi-arrows-angle-expand" title="{{$item.expand}}"></i>{{$item.expand}}</a>
              {{/if}}
